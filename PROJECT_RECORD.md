...
|---|---|---|---|---|
| **stage 1, satellite inputs** | **0.9006 °C** (`seafloor_masked_v1`, 2026-09-07; 0.9078 under `unmasked_v1`) | **+0.2400** (+0.1494 where the climatology is real) | OSTIA / DUACS / SMOS-blend / GLOBCURRENT / CMEMS-wind | **SHIPPED — the PS deliverable** |
| stage 1, GLORYS inputs | 0.8743 °C (`seafloor_masked_v1`, seed 42) | +0.2622 | reanalysis | comparator only |
...
| **Phase 2 (v2)** | 2026-08-26 → 09-05 | **TS-Cast-NIO** — 3-D CNN satellite encoder → climatology-prior decoder → T + log-variance heads | **daily**, 388 steps 2025-06-01 → 2026-06-23 | frozen 2026-09-02; Argo RMSE **0.9006** on 962 profiles, satellite inputs (0.9078 as first frozen, under `unmasked_v1`) |
...
| `argo_daily_period.parquet` | — | 2025-06-01 → 2026-06-23 | T | **the 962 profiles behind every v2 number** — 12,829 depth comparisons under `unmasked_v1`, **12,736 under `seafloor_masked_v1`** (93 below the target's seafloor, declined) |
...
**Every RMSE, skill and bias in this table is `unmasked_v1`** — each run's own metrics JSON as it
was written. Only the shipped checkpoint has been re-scored under `seafloor_masked_v1`
(2026-09-07); its row carries both, and the rest are the record of what was measured, not current
claims. See the protocol note at the head of §8.
...
> **SCORING PROTOCOL CHANGE 2026-09-07.** Every figure in §8–§11 was compiled under `unmasked_v1`,
...
| overall, re-measured under `seafloor_masked_v1` (2026-09-07) | +0.0263 | +0.0267 | **−0.0028** | **+0.0167** | **NO** |
| **Arabian Sea** | +0.0424 | +0.0528 | +0.0071 | **+0.0341** | **YES** |
| Bay of Bengal | −0.0037 | −0.0376 | −0.0168 | **−0.0194** | YES (satellite is *better*) |

**The overall effect does not survive the scoring fix.** Re-scored under `seafloor_masked_v1`,
seed 44 turns negative, so the sign no longer holds 3/3 and by this project's own rule the *overall*
satellite penalty is no longer an established effect — the two input sources are within seed noise
(mean +0.0167 °C, spread 0.0295). The per-basin split below was measured under `unmasked_v1` and has
**not** been re-run; it is reported as the record of that measurement, not as a current claim.
...
6.9e-4 °C is float32 noise against a 0.90 °C headline. The model is moved **back** in the
...
|---|---|---|---|
| `deliverable_satellite` | **THE PS DELIVERABLE** | 0.9006 (`seafloor_masked_v1`; 0.9078 as first frozen, under `unmasked_v1`) | yes, sha verified |
...
```
TEMPERATURE -- the only quantity with a stage-1 number to beat   [unmasked_v1 scoring]
...
contradiction, and not comparable. **[VERIFIED]**
